...
namespace EFDataModel.XIFIN_LIS
{
    public partial class XifinLISEntities : DbContext
    {
        ...
        public IObjectContextAdapter xifinLisContext { get; private set; }

        public XifinLISEntities(string conn)
            : base(conn)
        {
        }
            ...
            foreach (ObjectStateEntry entry in objectStateAddedEntryList)
            {
                if (!entry.IsRelationship && entry != null)
                {
                    ...
                    foreach (var propertyName in current.DataRecordInfo.FieldMetadata)
                    {
                        ...
                        {
                            userName = System.Security.Principal.WindowsIdentity.GetCurrent().Name ?? "XifinLISEntities",
                            source = "",
